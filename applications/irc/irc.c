...
static int (*docommand[LISTSIZE]) () =
{
	...
	donick, nop, donotice, nop, dopart, nop, doping, dopong, doprivmsg,
...
int parsedata()
{
    ...
    /* Handle PING and challenges BEFORE tokenization replaces spaces with \0 */
    if (strncmp(serverdata, "PING", 4) == 0) {
	char *p = serverdata + 4;
	while (*p == ' ' || *p == ':') p++;
	sprintf(lineout, "PONG :%s\n", p);
	sendline();
    ...
    if (serverdata[0] == ':' && strstr(serverdata, " PING ") != NULL) {
	char *p = strstr(serverdata, " PING ");
	p += 6;
	while (*p == ' ' || *p == ':') p++;
	sprintf(lineout, "PONG :%s\n", p);
	sendline();
    ...
    if (strstr(serverdata, "PONG") != NULL && strstr(serverdata, "513") != NULL) {
	char *p = strstr(serverdata, "PONG");
	p += 4;
	while (*p == ' ' || *p == ':') p++;
	char cookie[64] = "";
...
static void alarm_dummy(int sig) { (void)sig; }
...
{
    printf("Usage: %s [options] [nickname] [channel] [server] [port]\n\n", progname);
    printf("Lightweight terminal IRC client for SIX\n\n");
    printf("Options:\n");
    printf("  -u, -n <nick>     Set IRC nickname (default: current login user)\n");
    printf("  -c <channel>      Autojoin channel on connect (e.g. -c #six)\n");
    printf("  -s <server>       Specify IRC server (bypasses random pool)\n");
    printf("  -p <port>         Specify port number (default: 6667)\n");
    printf("  -d, --dumb        Dumb terminal mode (line-by-line, no split screen)\n");
    printf("  -h, --help        Display this help message\n\n");
    printf("Server Pool:\n");
    printf("  When no server is specified, irc connects randomly to one of:\n");
    printf("    - irc.efnet.org (port 6667)\n");
    printf("    - irc.undernet.org (port 6667)\n");
    printf("    - irc.dal.net (port 6667)\n");
    printf("  If the chosen server is unreachable, it automatically fails over\n");
    printf("  to the next available server in the pool.\n\n");
    printf("Examples:\n");
    printf("  %s                           Quick connect with default user to random server\n", progname);
    printf("  %s #six                      Connect with default user and join #six\n", progname);
    printf("  %s joker                     Connect as 'joker' to random server\n", progname);
    printf("  %s joker #six                Connect as 'joker' and join #six\n", progname);
    printf("  %s joker irc.efnet.org 6667  Positional syntax: nick server port\n", progname);
    printf("  %s -u joker -s 127.0.0.1     Structured flags syntax\n\n", progname);
}
...
For details please see the file COPYING.\n", RELEASE);
    ...
    /* Initialize default user identity from passwd/ldap */
    userinfo = getpwuid(getuid());
    if (userinfo != NULL) {
	strcpy(IRCLOGIN, userinfo->pw_name);
	tmp = (char *) getenv("IRCNICK");
	if (tmp != NULL && *tmp)
	    strncpy(IRCNAME, tmp, sizeof(IRCNAME) - 1);
	else
	    strncpy(IRCNAME, userinfo->pw_name, sizeof(IRCNAME) - 1);
	tmp = userinfo->pw_gecos;
	if (tmp != NULL && *tmp) {
	    strncpy(IRCGECOS, tmp, sizeof(IRCGECOS) - 1);
	    if ((tmp = strchr(IRCGECOS, ','))) *tmp = '\0';
	} else {
	    strncpy(IRCGECOS, userinfo->pw_name, sizeof(IRCGECOS) - 1);
	}
    } else {
	strcpy(IRCLOGIN, "user");
	strcpy(IRCNAME, "user");
	strcpy(IRCGECOS, "SIX User");
    }
    ...
    /* Parse structured flags and positional arguments */
    for (i = 1; i < argc; i++) {
	if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
	    print_help(argv[0]);
	    exit(0);
	} else if (strcmp(argv[i], "-d") == 0 || strcmp(argv[i], "--dumb") == 0) {
	    dumb = 1;
	} else if (strcmp(argv[i], "-u") == 0 || strcmp(argv[i], "-n") == 0) {
	    if (i + 1 < argc) {
		strncpy(IRCNAME, argv[++i], sizeof(IRCNAME) - 1);
	    }
	    ...
	    if (i + 1 < argc) {
		IRCPORT = (unsigned short) atoi(argv[++i]);
	    }
	} else if (strcmp(argv[i], "-c") == 0) {
	    if (i + 1 < argc) {
		strncpy(autojoin_chan, argv[++i], sizeof(autojoin_chan) - 1);
	    }
	} else if (argv[i][0] == '#' || argv[i][0] == '&') {
	    /* Channel argument */
	    strncpy(autojoin_chan, argv[i], sizeof(autojoin_chan) - 1);
	} else if (argv[i][0] != '-') {
	    /* Positional arguments: pos 0 = nick, pos 1 = server, pos 2 = port */
	    if (pos == 0) {
		strncpy(IRCNAME, argv[i], sizeof(IRCNAME) - 1);
		pos++;
	    } else if (pos == 1) {
	...
	int pick = rand() % 2; /* 0 = dal.net, 1 = esper.net */
	int order[5];
	order[0] = pick;
	order[1] = 1 - pick;
	order[2] = 2; /* supernets */
	...
	for (i = 0; i < SERVER_POOL_SIZE; i++) {
	    const char *candidate = server_pool[order[i]];
	    printf("*** Trying %s (port %d)...\n", candidate, IRCPORT);
	    fflush(stdout);
	    strcpy(hostname, candidate);
	    if (makeconnect(hostname) >= 0) {
		printf("*** Connected to %s!\n\n", hostname);
		connected = 1;
		break;
	    }
	    printf("*** Could not connect to %s, trying next server...\n", candidate);
	}
	if (!connected) {
	    fprintf(stderr, "*** All IRC servers failed to connect, aborting.\n");
	    exit(1);
	}
    } else {
