...
void Proc0()
{
  ...
  printf("Running Dhrystone (%s) benchmark (%d seconds)...\n", Version, SECONDS);
  fflush(stdout);
...
-- Start Timer --
*****************/
  /* Align to the start of a fresh second tick */
  nulltime = (unsigned long)time((time_t *)0);
  while ((starttime = (unsigned long)time((time_t *)0)) == nulltime)
	;
  ...
  for (;;) {
	...
	Proc2(&IntLoc1);
	if ((i & 0x3fff) == 0) {
		benchtime = (unsigned long)time((time_t *)0) - starttime;
		if (benchtime >= SECONDS)
			break;
	}
  }
  ...
  if (benchtime == 0)
	benchtime = 1;
  printf("Dhrystone(%s) time for %lu passes = %lu.00s\n",
	Version, benchloops, benchtime);
  printf("This machine benchmarks at %lu dhrystones/second (~%lu VAX MIPS)\n",
	benchloops / benchtime, (benchloops / benchtime) / 1757);
}
