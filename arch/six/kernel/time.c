...
*/
        ...
        save_freq_select = CMOS_READ(RTC_FREQ_SELECT); /* stop and reset prescal
er */
        ...
        if (((abs(real_minutes - cmos_minutes) + 15)/30) & 1)
                real_minutes += 30;             /* correct for half hour time zo
ne */
        real_minutes %= 60;
 ...
 */
#if (SIX)
static inline unsigned long mk_time(unsigned int year, unsigned int mon,
#else
...
unsigned long get_cmos_time(void)
{
        ...
        if ((year += 1900) < 1970)
                year += 100;
#if (SIX)
        return mk_time(year, mon, day, hour, min, sec);
#else
