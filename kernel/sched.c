...
/*
 * tq_immediate: this is really a bottom half IMMEDIATE_BH, so drivers can
...
struct task_struct *mapped_proc;
#endif
...
static unsigned long init_kernel_stack[DEFAULT_STACK_SIZE] = { STACK_MAGIC, };
 ...
 *	a) on each schedule
 *	b) when a systemcall/interrupt returns 
 *
 */
