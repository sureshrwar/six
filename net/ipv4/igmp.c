...
static __inline__ unsigned int random(void)
{
...
static __inline__ void ip_mc_map(unsigned long addr, char *buf)
{
	addr=ntohl(addr);
	buf[0]=0x01;
	buf[1]=0x00;
...
static __inline__ void igmp_group_dropped(struct ip_mc_list *im)
{
	del_timer(&im->timer);
...
static __inline__ void igmp_group_added(struct ip_mc_list *im)
{
	struct ip_router_info *r;
	igmp_init_timer(im);
