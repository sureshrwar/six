...
#if (SIX)
#define DEFAULT_MULT_COUNT  2   /* set to 0 to disable multiple mode at boot */
#else
...
/*
#if (__i386__)
#define		DISKFILE	"/home/warrier/six/intel/six/disk/x86/root"
#else
#define		DISKFILE	"/six/src/six/disk/sparc/root"
#endif
*/
...
unsigned short wdisk_buffer[512];
...
ok_to_write:
        ...
        if (i > 0) {
                SET_INTR(&write_intr);
#if (SIX)
                outsw(HD_DATA,CURRENT->buffer,512);
                end_request(1);
#else
...
{
	/*
	 * if the guy out there doesnt know wat to do with
...
repeat: 
	...
	INIT_REQUEST;
        ...
        if (CURRENT->cmd == WRITE) {
                ...
                hd_out(dev,nsect,sec,head,cyl,cmd,&write_intr);
                if (reset)
                        goto repeat;
#endif
                return;
...
/*
#if (SIX)
	if(!access(DISKFILE, 0))
	{
		DISKFD = open(DISKFILE, O_RDWR);
		if(DISKFD < 0)
			panic("cant open root disk");
	}
	else
		panic("cant locate root disk!");
#endif
*/
}
...
{
	wdisk_buffer[wbuf_offset++] = val;
}

unsigned short get_hard_data()
{
	return disk_buffer[buf_offset++];
}
#endif
