...
int loopback_init(struct device *dev)
{
	...
	dev->flags		= IFF_LOOPBACK|IFF_BROADCAST|IFF_UP|IFF_RUNNING;
	dev->family		= AF_INET;
#ifdef CONFIG_INET    
...
#ifdef CONFIG_INET
	{
		...
		((struct sockaddr_in *)&rt.rt_dst)->sin_addr.s_addr = in_aton("127.0.0.0");
		((struct sockaddr_in *)&rt.rt_genmask)->sin_family = AF_INET;
		((struct sockaddr_in *)&rt.rt_genmask)->sin_addr.s_addr = in_aton("255.0.0.0");
		rt.rt_flags = RTF_UP;
		rt.rt_dev = NULL;
		ip_rt_new(&rt);
	}
