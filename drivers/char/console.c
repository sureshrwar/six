...
#if (SIX)
void sun_gotoxy(int xx, int yy)
{
	char string[20];
	sprintf(string, "%c[%d;%dH", 27, xx+1, yy+1);
...
void find_position(long mark)
{
	int temp, row, col;
	temp = (mark - video_mem_base) >> 1;
...
void scrup(int currcons, unsigned int t, unsigned int b)
{
        ...
        if (b > video_num_lines || t >= b)
                return;
#if (!SIX)	/* let hard scroll be zero always */
...
unsigned long con_init(unsigned long kmem_start)
{
	...
	printk("Console init: memory start shifted to 0x%x\n", kmem_start);
