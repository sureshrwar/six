...
### Build Everything
A single `make` builds the `./six` kernel, the 32-bit guest `libc.a`, all 24 guest userland programs under `applications/`, and assembles the root `ext2` disk image (`disk/x86/root` via `port/image/mkimage.sh`):
...
* Included guest utilities in `/bin`: `banner`, `cat`, `clear`, `date`, `echo`, `fortune`, `getty`, `grep`, `halt`, `hello`, `id`, `init` (`/etc/init`), `kill`, `last`, `login`, `ls`, `ps`, `pwd`, `rm`, `sethostname`, `sh` (with `~/.bash_history` and Up/Down arrow recall), `sync`, `tty`.
