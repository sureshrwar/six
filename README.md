...
* **Emulated Physical RAM & Software MMU (`init/main.c`, `arch/six/mm/init.c`, `mm/mmap.c`)**:
  * **Physical RAM (`RAMFD`)**: At boot, `grow_ram()` creates a 32 MB unlinked temporary file (`RAMFD = fileno(tmpfile())`) and maps it `MAP_SHARED | MAP_FIXED_NOREPLACE` at a low 32-bit base address `_ram_start` (above the host `brk` ASLR window). Linux 2.0.11's `mem_map[]` and buddy allocator (`get_free_page()`) carve 4 KB physical page frames out of `[_ram_start, _ram_start + 32 MB)`, so any physical page's file offset in `RAMFD` is `page - _ram_start`.
  * **Software MMU & Address-Space Switching (`set_proc_mappings`)**: Guest ELF32 binaries are linked at virtual address `0x03000000` with stacks below `STACK_BASE` (`0x10000000`). Each process maintains its own Linux 2.0.11 VMA list (`mm->mmap`) and page tables (`pgd`/`pmd`/`pte`). When mappings are created (`do_mmap`, `sys_brk`, `execve`) and on every context switch between user processes (`switch_to` → `set_proc_mappings()`), `map_page_range()` / `map_one_pte()` walks the incoming task's page tables and calls host `mmap(vaddr, PAGE_SIZE, ..., MAP_SHARED | MAP_FIXED, RAMFD, pte_page(*pte) - _ram_start)` to overlay that task's physical frames from `RAMFD` onto the guest virtual address range.
