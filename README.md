...
### Build Everything
A single `make` builds the `./six` kernel, the 32-bit guest `libc.a`, all 36 guest userland programs under `applications/`, and assembles the root `ext2` disk image (`disk/x86/root` via `port/image/mkimage.sh`):
...
* Included guest utilities in `/bin`: `advent` (*Colossal Cave Adventure*), `banner`, `basic` (interactive Dartmouth/Tiny BASIC interpreter), `cal`, `cat`, `clear`, `date`, `dhrystone` (Dhrystone 1.1 benchmark), `echo`, `eliza` (classic 1966 Rogerian psychotherapist chatbot), `fortune`, `getty`, `gomoku` (Five-in-a-Row), `grep`, `halt`, `hello`, `id`, `init` (`/etc/init`), `kill`, `last`, `life` (Conway's Game of Life), `login`, `ls`, `matrix` (Matrix digital rain screensaver), `ps`, `pwd`, `rm`, `rogue` (classic BSD-style dungeon crawler), `sethostname`, `sh` (Minix Bourne shell with `~/.bash_history` and Up/Down arrow recall), `sync`, `tetris` (colored ANSI Tetris), `ttt` (Tic-Tac-Toe), `tty`, `vi` (`elvis`).
