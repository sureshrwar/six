...
#if (SIX)
static inline void free_area_pte(pmd_t * pmd, unsigned long address, unsigned long size, unsigned long base)
#else
...
static inline void free_area_pmd(pgd_t * dir, unsigned long address, unsigned long size)
{
        ...
        unsigned long end;
#if (SIX)
	unsigned int base = address & PGDIR_MASK;
#endif
        ...
        while (address < end) {
#if (SIX)
                free_area_pte(pmd, address, end - address, base);
#else
...
#if (SIX)
static inline int alloc_area_pte(pte_t * pte, unsigned long address, unsigned long size, unsigned long base)
#else
...
#if (SIX)
		ret = (void *) mmap(address + base, PAGE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC,
		                    MAP_SHARED | MAP_FIXED, RAMFD, pte_page(*pte) - _ram_start);
		if (ret != (address + base))
			return -ENOMEM;
#endif
                address += PAGE_SIZE;
	...
	base = (address & PGDIR_MASK);
        ...
        while (address < end) {
                ...
                if (!pte)
                        return -ENOMEM;
#if (SIX)
                if (alloc_area_pte(pte, address, end - address, base))
#else
...
#if (SIX)
		mmap(offset, PAGE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC,
		     MAP_SHARED | MAP_FIXED, RAMFD, pte_page(*pte) - _ram_start);
#endif
                address += PAGE_SIZE;
                offset += PAGE_SIZE;
