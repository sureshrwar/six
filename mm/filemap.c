...
#if (SIX)
	/*
	 * Ok we are called from inside do_mmap(). Before that, do_six_mmap() would have
	 * been called, and it would have setup the required memory in vma->real. So now
	 * we have the inode, we have the offset, we have the memory and we know the size.
	 * so just copy the required amount of data.
	 */
	read_exec(inode, vma->vm_offset, vma->real, vma->vm_end - vma->vm_start);
#endif
	return 0;
}
