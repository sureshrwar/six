 ...
 *  Torbj\366rn Lindh (torbjorn.lindh@gopta.se), April 14, 1996.
 *
        ...
        if (MAJOR(ROOT_DEV) == UNNAMED_MAJOR) {
                ...
                if ((fs_type = get_fs_type("nfs"))) {
                        ...
                        if (nfs_root_mount(sb) >= 0) {
                                ...
                                ROOT_DEV = sb->s_dev;
                                printk (KERN_NOTICE "VFS: Mounted root (nfs file
system).\n");
                                vfsmnt = add_vfsmnt(ROOT_DEV, "rootfs", "/");
                                if (!vfsmnt)
                                        panic("VFS: add_vfsmnt failed for NFS ro
ot.\n");
                                vfsmnt->mnt_sb = sb;
                                vfsmnt->mnt_flags = sb->s_flags;
                                return;
                ...
                if (!ROOT_DEV) {
                        printk(KERN_ERR "VFS: Unable to mount root fs via NFS, t
rying floppy.\n");
                        ROOT_DEV = MKDEV(FLOPPY_MAJOR, 0);
                }
        }
        ...
        if (retval)
                ...
                if (sb) {
                        inode = sb->s_mounted;
                        inode->i_count += 3 ;   /* NOTE! it is logically used 4
times, not 1 */
                        sb->s_covered = inode;
