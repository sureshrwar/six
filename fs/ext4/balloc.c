...
unsigned long ext4_count_free_blocks (struct super_block * sb)
{
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
		...
		bitmap_nr = load_block_bitmap (sb, i);
		x = ext4_count_free (sb->u.ext2_sb.s_block_bitmap[bitmap_nr],
				     sb->s_blocksize);
		printk ("group %d: stored = %d, counted = %lu\n",
			i, gdp->bg_free_blocks_count, x);
		bitmap_count += x;
	}
	printk("ext4_count_free_blocks: stored = %lu, computed = %lu, %lu\n",
...
{
#if (SIX && (!__i386__))
	return ext2_test_bit ((block - sb->u.ext2_sb.s_es->s_first_data_block) %
